...
function Post({ props, onClickShare, lastBlogElementRef }) {
    ...
    const [homeTags, updateHomeTags] = useState([]);
    ...
    return (
        <div key={props.id} ref={lastBlogElementRef} className="body">
            {
                modalOpen &&
                <Modal setModalOpen={setModalOpen} modalClassName='share_modal' children={<Share props={props} />} />
            }
                ...
                <div className="panel-heading">
                    <div className="media">
                        <div className="media-left">
                            <a href="#">
                                <img width="42" height="42" src={`${props.avatar ? `${process.env.REACT_APP_ASSETS_URL}/uploads/avatar/${props.avatar}` : ''}`} />
                            </a>
                        </div>
                ...
                <div className="panel-body">
                    <div className="post">
                        <div className="post-heading">
                            <div className="tags blogs_tags" >
                                <div className="swiper-wrapper pills tag hero_tag">
                                    {homeTags && homeTags.length > 0 &&
                                        homeTags.map((item) => {
                                            return (
                                                <Link to={`/tags/${item.slug}`} href={item.slug} className="pills_item pills_item_post swiper-slide">{item.name}</Link>
                            ...
                            <h2 className="post-title"><Link to={`/${props.slug}`}>{props.title}</Link></h2>
