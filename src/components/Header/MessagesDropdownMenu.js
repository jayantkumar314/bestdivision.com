...
import useMessageSearch from './useMessagesSearch'
...
function MessagesDropdownMenu() {
    const [activeMenu, setActiveMenu] = useState('main');
    const [menuHeight, setMenuHeight] = useState(null);
    const dropdownRef = useRef(null);
    const [params, setParams] = useState(initialParams)
    const { loading, error, notifications, hasMore } = useMessageSearch(params)
    ...
    function DropdownItem({props, lastMessageElementRef}) {
        return (
            <>
                ...
                <li key={props.id} ref={lastMessageElementRef} >
                        ...
                        {notifications && notifications.length > 0 &&
                            notifications.map((notification, index) => {
                                if (notifications.length === index + 1) {
                                    return (<DropdownItem props={notification} lastMessageElementRef={lastMessageElementRef} />);
                                } else {
